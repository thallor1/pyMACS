...
testobj = virtualMACS('CrI3_monotest',cifName='CrI3.cif',useOld=False)
...
#Set a sample diameter for correct monochromator calculations. 
...
testobj.sample.orient_u=[1,0,0]
testobj.sample.orient_v=[-1,2,0]
...
testobj.script_scan(A3_list=np.arange(-70,-20,0.25),Ei_list=[5.0],\
                    num_threads=12,scan_title='_cri3_simplescan_cube')

testobj.data.load_data_matrix_from_csv('_cri3_simplescan_cube_dataMatrix.csv')
testobj.data.write_data_to_ng0('_cri3_simplescan_cube.ng0')
...
plt.xlabel('[HH0]')
plt.ylabel('[00L]')
plt.title("CrI3 A3 Scan, Elastic")
plt.savefig("CrI3_A3scan_test.pdf",bbox_inches="tight")